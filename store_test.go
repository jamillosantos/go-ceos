	...
	Describe("BaseStore", func() {
		Context("Insert", func() {
			...
			It("should insert a user specifying no fields", func() {
				...
				rs, err := db.NewUserResultSet(b.RunWith(db.DB).Query())
				Expect(err).ToNot(HaveOccurred())
				defer rs.Close()
				Expect(rs.Next()).To(BeTrue())
				Expect(rs.Scan()).To(Succeed())
				Expect(rs.Model.Name).To(Equal("Snake Eyes"))
				Expect(rs.Model.Password).To(Equal("12345"))
				Expect(rs.Model.Role).To(Equal("stealth"))
			...
			It("should insert multiple users returning the PK", func() {
				...
				Expect(rs.Next()).To(BeTrue())
				Expect(rs.Scan()).To(Succeed())
				Expect(rs.Model.Name).To(Equal("Scarlet"))
				Expect(rs.Model.Password).To(Equal("54321"))
				Expect(rs.Model.Role).To(Equal("intelligence"))
				Expect(rs.Next()).To(BeFalse())
			})
			...
			It("should insert a user specifying fields", func() {
				...
				Expect(rs.Scan()).To(Succeed())
				Expect(rs.Model.Name).To(Equal("Snake Eyes"))
				Expect(rs.Model.Password).To(Equal(""))
				Expect(rs.Model.Role).To(Equal(""))
				Expect(rs.Next()).To(BeFalse())
			})
			...
			It("should insert a model with a composite PK", func() {
				...
				Expect(rs.Model.ID.GroupID).To(Equal(2))
				Expect(rs.Model.Admin).To(BeTrue())
				Expect(rs.Next()).To(BeFalse())
			})
		})
